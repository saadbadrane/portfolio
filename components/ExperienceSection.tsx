        ...
        >
          {t.experience.list.map((exp, index) => (
            <motion.div key={index} variants={item} className="relative pl-8 md:pl-12">
              {/* Timeline Dot */}
              <span className="absolute -left-[5px] top-2 h-2.5 w-2.5 rounded-full bg-orange-600 dark:bg-orange-400 ring-4 ring-background" />
              
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
                <h3 className="text-xl font-bold text-foreground">{exp.role}</h3>
                <span className="text-sm text-muted-foreground font-medium bg-background px-2 py-1 rounded border border-orange-600 dark:border-orange-400 shadow-[0_2px_10px_rgba(37,99,235,0.15)] dark:shadow-[0_2px_10px_rgba(96,165,250,0.15)] mt-1 sm:mt-0 w-fit">
                  {exp.dates}
                </span>
              </div>
